...
public aspect EcoreAspect_SystemItemProvider {
	...
	after(Notification notification) returning: execution (* (todolist.provider.SystemItemProvider).notifyChanged(..)) && args(notification) {
		Object imbType;
		Object oldValue;
		Object newValue;
		Object notifier;
		...
		switch(notification.getEventType()) {
			case Notification.ADD:
				if(EReference.class.isAssignableFrom(notification.getFeature().getClass())) {
					try {
						...
						imbType = EcoreAspect_SystemItemProvider. getImbType(referenceName, newValue);
			...
			case Notification.SET:
				try {
					...
					imbType = EcoreAspect_SystemItemProvider. getImbType(containingClass, notifier);
